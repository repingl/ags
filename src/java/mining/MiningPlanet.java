	...
	public boolean executeAction(String ag, Structure action){
		...
		try{
			...
			if(action.equals(up)){
				...
				result = model.transfer(agId, model.getAgIdBasedOnName(action.getTerm(1).toString()),
						Integer.parseInt(action.getTerm(2).toString()));
			}else{
			...
			if(result.isNotError()) // nepodvadel nekdo?
			{
				...
				if(result == ActionResult.ROUND_FINISHED){
					for(int i = 0; i < 6; i ++){
						...
						addPercept(model.getAgNameBasedOnId(i),
								Literal.parseLiteral("moves_left(" + model.leftMovesGet(i) + ")"));
					}
	...
	public void initWorld(int w){
		...
		try{
			...
			case 4:
				...
				break;
			case 5:
				model = WorldModel.world5();
				break;
			case 6:
				model = WorldModel.world6();
				break;
